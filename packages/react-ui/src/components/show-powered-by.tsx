...
// Access the brand name from the injected HTML data
const getBrandName = (): string => {
  // @ts-expect-error - accessing injected global variable
  return window.apBrandName || 'activepieces';
};
    ...
    >
      ...
      >
        ...
        <div className="justify-center flex items-center gap-1">
          ...
          <div className="font-semibold">{getBrandName()}</div>
        </div>
      </div>
    </div>
  );
